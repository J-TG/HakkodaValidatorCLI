...
def ensure_config_defaults() -> None:
    ...
    st.session_state.setdefault("compat_mapping_db", "STAGE_TEST")
    st.session_state.setdefault("compat_mapping_schema", "SCANDS_FINANCE")
    st.session_state.setdefault("compat_finance_schema", "SCANDS_FINANCE")
    st.session_state.setdefault("compat_prod_schema", "SCANDS_PROD")
    st.session_state.setdefault("compat_quality_schema", "SCANDS_QUALITYRISK")
...
def main() -> None:
    ...
    with st.expander("Builder Configuration", expanded=True):
        with st.form("compat_config_form"):
            ...
            with col2:
                target_db = st.text_input("Target Database (source tables)", value=st.session_state.get("compat_target_db", "STAGE_TEST"))
